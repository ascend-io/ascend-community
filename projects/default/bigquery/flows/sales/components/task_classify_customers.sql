...
WITH combined_sales AS (
  ...
  SELECT 
    CAST(ascender_id AS STRING) as ascender_id,
    timestamp,
    price
  FROM {{ ref("sales") }}
...
customer_purchase_stats AS (
  ...
  SELECT 
    ...
    DATE_DIFF(MAX(timestamp), MIN(timestamp), DAY) as customer_lifespan_days,
    -- Calculate average days between purchases
    CASE 
      WHEN COUNT(*) > 1 THEN 
        DATE_DIFF(MAX(timestamp), MIN(timestamp), DAY) / NULLIF(COUNT(*) - 1, 0)
      ELSE NULL 
